...
import PageWrapper from '@/components/PageWrapperRegister';
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!firstName || !lastName || !employeeNumber || !email || !password || !confirmPassword) {
      setLoading(false);
      toast.error('Semua field harus diisi.'); // ✅ pakai toast
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Registrasi berhasil! Silakan login.'); // ✅ toast success
        setTimeout(() => {
          ...
          <div className="max-w-md w-full bg-white/10 backdrop-blur-md rounded-xl p-8 text-white shadow-lg">
            ...
            <form className="space-y-5" onSubmit={handleRegister}>
              ...
              <p className="text-sm text-center mt-4">
                ...
                <a href="/login" className="font-semibold text-white underline cursor-pointer">
                  Login here
                </a>
              </p>
            </form>
